...
const getAllItems = async ()=> {
    try {
        const allItems = await db.any("SELECT * FROM todoItems, to_char(due_date, 'Month DD, YYYY') AS formatted_due_date, to_char(time_of_day,'HH12:MI AM') AS formatted_time")
        return allItems;
    } catch (error) {
        return error;
    }
